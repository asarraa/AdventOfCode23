...
public class Puzzle3 {
    ...
    public void symbolsAssigner(String line){
        if(firstTime==true){
            this.symbols=new Star[160][line.length()+10];
        ...
        {
            if(line.charAt(i)=='*'){
                this.symbols[actualLineNumber][i]=new Star();
            }
            else this.symbols[actualLineNumber][i]=null;
            i++;
        }
            ...
            if(actualLineNumber!=0 && actualLineNumber!=max) 
            {
                for(i=limits[0]-1; i<=limits[1]+1 && isAdjacent==false; i++){      //per tutta la lunghezza del numero + gli adiacenti
                    for(int j=actualLineNumber-1; j<=actualLineNumber+1 && isAdjacent==false; j++){  //per la linea precedente, la linea attuale e la linea successiva
                        if(this.symbols[j][i]!=null)
                        {
                            if(this.symbols[j][i].freeSpaces>-1){
                                Star asterisco=this.symbols[j][i];
                                asterisco.adjacents[asterisco.freeSpaces]=limits[2];
                                asterisco.freeSpaces--;
                            }
                                    if(this.symbols[j][i].freeSpaces==-1){
                                        Star asterisco=this.symbols[j][i];
                                        sum=sum+asterisco.multiplyValues();
                                    }                            
                        }
                    }
                }
